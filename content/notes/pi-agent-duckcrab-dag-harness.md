...
## 목표는 DAG 플랫폼이 아니라 전문가 작업공간입니다

DAG가 무엇인지 알았으니 목표를 분명히 할 수 있습니다. 우리가 만들려는 것은 “DAG를 사용하는 멀티에이전트 플랫폼” 자체가 아닙니다.

우리가 필요한 것은 사용자의 질문을 받으면 조직의 실제 자료를 읽고, 근거와 반례를 비교하고, 부족하면 멈출 수 있는 **질문별 전문가 작업공간**입니다.
...
DAG와 Pi Agent는 이 흐름을 구현하기 위해 선택한 수단입니다. 역할을 나누면 다음과 같습니다.
...
즉, DAG는 목표가 아니라 전문가 작업공간 안에서 조사 절차를 빠뜨리지 않게 만드는 도구입니다. 따라서 이 글의 중심 질문도 “Pi가 좋은가?”나 “DAG 엔진을 어떻게 만드는가?”가 아닙니다.

> **10번 글의 Expertise Pack을 실제 질문별 전문가 작업공간으로 바꾸려면 무엇이 더 필요한가?**

## Expertise Pack에는 무엇이 들어가야 하는가

Expertise Pack을 단순한 문서 모음으로 생각하면, 검색한 관련 자료를 답변에 붙이는 일반 RAG(검색 증강 생성)와 크게 다르지 않습니다. 좋은 Pack에는 문서뿐 아니라 다음 정보가 서로 구분돼 있어야 합니다.
...
우리는 처음부터 시작하는 것이 아닙니다. 이 글에서는 **OpenCrab을 의미 설계와 Pack 구조의 출발점**, **DuckCrab을 그 설계를 로컬 정본·검색·검증 흐름으로 옮긴 현재 구현**으로 구분해 부르겠습니다.
...
이 네 계약을 먼저 정해 두면, 이제 공개 Pi 패키지를 기능 목록이 아니라 **필요한 계약을 얼마나 대신 구현해 주는가**라는 기준으로 비교할 수 있습니다.
...
따라서 패키지는 실행 기반을 줄여 줄 수는 있어도, 전문가가 어떤 자료를 확인해야 조사를 마칠 수 있는지까지 정해 주지는 않습니다.
