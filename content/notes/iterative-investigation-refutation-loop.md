...
예를 들어 “서비스 장애의 원인은 새 배포다”라는 가설을 세웠다고 해보겠습니다. 조사 루프는 배포 문제를 뒷받침할 자료만 더 모으는 대신 외부 API 장애라는 경쟁 설명과 둘을 구분할 관찰을 찾습니다.
...
이 일곱 항목은 매 반복에서 남겨야 할 최소 기록입니다. 여기서 `hypothesis`는 하나의 결론만 뜻하지 않으며, 현재 주 가설과 이를 구분해야 할 경쟁 가설을 함께 연결합니다.
...
Self-RAG는 검색이 필요하지 않거나 관련 없는 구절을 고정 개수로 넣는 방식이 생성을 해칠 수 있다고 지적하고, 필요할 때 검색하도록 학습했습니다.[src_008](#src-008) 2026년 다중 턴 검색 연구는 긴 문맥의 무관한 문서가 모델을 방해하는 문맥 간섭(`context interference`)을 분석했으며, 특히 최신 검색 문서가 주요 간섭원이 될 수 있다고 보고했습니다.[src_014](#src-014)
...
앞에서 살펴본 실패와 필요한 통제를 한눈에 정리하면 다음과 같습니다. 검색 오염과 평가 착시는 조사 과정뿐 아니라 평가 단계에서도 따로 확인해야 할 위험입니다.
...
이를 Expertise Pack·문맥 컴파일러·Validator와 연결하는 전체 구조는 다음 13번 글에서 다룹니다. 여기서는 그 구조 안에서 첫 가설과 계획을 다시 검사하는 루프에만 집중합니다. 이 연결 역시 현재 구현된 기능이 아니라 후속 구현과 비교 실험으로 검증할 설계 가설입니다.
