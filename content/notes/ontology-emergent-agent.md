...
description: "온톨로지 변경이 에이전트의 지속적인 행동 변화로 이어졌다고 말하려면 어떤 아키텍처, 인과 실험, 검증·승격·롤백 체계가 필요한지 3인 토론 결과로 정리합니다."
date: 2026-07-20
tags:
  - 온톨로지
  - AI에이전트
...
“온톨로지가 스스로 진화하면서 에이전트의 기억과 행동까지 바꾼다면, 그것을 정말 새로운 종류의 에이전트라고 부를 수 있을까?” 이 질문을 놓고 같은 연구 Pack을 세 가지 관점으로 읽고, 별도 리뷰어가 두 차례 반박과 수정을 거치게 했다. 결론은 명확했다. **그래프를 검색해 답변이 달라지는 것만으로는 온톨로지 창발이 아니다. 온톨로지의 변경이 실행 규칙으로 컴파일되고, 그 결과가 재시작 후에도 지속되며, 롤백하면 이전 행동까지 돌아가야 한다.**
...
토론의 첫 라운드는 여기서 실패했다. 좋은 구성요소는 많이 나왔지만 “창발”이 무엇인지 반증 가능한 정의가 없었다. 두 번째 라운드에서 다음 판정식을 고정했다.
...
```text
Δ Ontology
  → Δ Compiled Runtime Artifact
  → Persistent Δ Behavior
  → Rollback
  → Prior Artifact and Behavior Restored
```
...
토론에는 세 개의 렌즈를 사용했다. 첫 번째는 의미 아키텍처와 장기 메모리, 두 번째는 평가 인식론과 Judge 신뢰성, 세 번째는 거버넌스와 운영이었다. 세 답변이 합의한 시스템은 다음 아홉 구성요소로 정리된다.

```text
Immutable Evidence Plane
        │ exact spans, hashes, events
        ▼
Typed-Patch Proposer ──► Candidate Quarantine
        │ proposal only
        ▼
Deterministic + Epistemic Validator
        │ validated version only
        ▼
Versioned Ontology Store
        │ pinned ontology version
        ▼
Deterministic Ontology Compiler
        │ signed artifact + source map
        ▼
Read-only Agent Runtime
        │ action and behavior trace
        ▼
Evaluator / Canary / Monitoring
        │
        ▼
Deployment Controller: Promote or Rollback

Every transition ──► Immutable Receipt Ledger
```
...
변경안을 여러 LLM Judge에게 평가시키는 것은 도움이 될 수 있지만, 표의 수와 독립 증거의 수는 다르다. PoLL은 다양한 모델 panel이 일부 설정에서 사람 선호 합의와 비용을 개선할 수 있음을 보였다. 반면 9개 Judge, 7개 model family를 분석한 2026년 프리프린트는 유효 독립표가 약 `2.18`에 불과하고, 독립성을 가정한 예측과 실제 다수결 사이에 큰 Condorcet gap이 생길 수 있음을 보고했다.[src_007](#src-007) [src_008](#src-008)
...
토론에서 가장 중요한 정리는 “반드시 지켜야 할 구조”와 “운영하면서 보정할 숫자”를 분리한 것이다.
