...
## 6. Apollo를 보며 에이전틱 개발이 떠오른 이유

공장의 판단 기준을 바꾸는 과정에는 변경 요청, 격리된 시험, 현업 리허설, 적용 승인과 장애 대응이 이어집니다. 제가 Apollo의 운영 방식에서 에이전틱 개발을 떠올린 이유는 이 과정 때문입니다. **변경을 만들었다는 사실과 실제로 사용해도 된다는 판단 사이에 여러 단계가 필요합니다.**
...
앞의 공장에서는 한시 예외 조건이 잘 동작해도 품질 담당자가 승인 범위를 이해하는지 따로 확인했습니다. 개발에서도 에이전트가 함수를 고치고 테스트를 통과했다고 해서 다른 변경과 합친 화면이나 실제 사용자 경로까지 확인된 것은 아닙니다. 격리된 실험에서 확인한 근거에 통합 검증을 더하고, 그 결과를 보고 실제 사용을 허용하는 흐름이 닮았습니다.

이 흐름에서 **검증 중인 변경 → 통합해 확인한 변경 → 실제 사용을 허용한 변경**은 서로 다른 신뢰 단계입니다. 뒤 단계로 넘긴다는 것은 오류가 없다고 인증하는 일이 아니라, 확인한 근거에 따라 영향력을 허용할 범위와 조건을 넓히는 판단입니다. 코드 생성이 빨라질수록 개발자는 그 승격 조건과 실패 시 대응도 설계해야 합니다.

| Apollo·Foundry 운영에서 하는 일                      | 에이전틱 개발에서 대응하는 일                          | 함께 확인할 질문                                               |
| ---------------------------------------------------- | ------------------------------------------------------ | -------------------------------------------------------------- |
| 변경 케이스에 사유·영향·책임을 남긴다                | 이슈·PR에 문제·변경 범위·검토 책임을 남긴다            | 왜 바꾸며 누가 받아들일지 판단하는가                           |
| Dev에서 판단 기준과 액션을 시험한다                  | 작업 브랜치·격리된 환경에서 에이전트의 수정을 시험한다 | 실패가 실제 업무나 다른 작업에 바로 퍼지지 않는가              |
| Staging에서 통합 상태와 현업 사용을 리허설한다       | 통합 브랜치와 시험 환경에서 CI·E2E·리뷰를 수행한다     | 따로 통과한 변경들이 함께 사용돼도 맞는가                      |
| 검증 근거와 책임 합의를 거쳐 적용 범위를 넓힌다      | 검증된 PR의 병합·릴리스·배포를 판단한다                | 무엇을 근거로 더 넓은 사용을 허용하는가                        |
| 환경별 제약을 확인하고 적용 후 상태를 보고받는다     | 배포 조건을 확인하고 실제 실행 결과를 관찰한다         | 허용한 변경이 실제로 적용됐고 정상 작동하는가                  |
| 문제 Release에서 벗어나고 영향받은 업무를 재검토한다 | revert·재배포와 데이터·외부 작업의 후속 조치를 나눈다  | 코드 복구 뒤에도 무엇이 남는가                                 |
| 변경·장애 기록을 다음 판단의 근거로 남긴다           | PR·커밋·회귀 테스트·인계 문서로 교훈을 남긴다          | 다음 작업이 같은 실패를 반복하지 않으려면 무엇을 알아야 하는가 |

이 대응은 각 단계가 맡는 역할을 나란히 놓은 것입니다. Git 브랜치는 코드 revision을 가리키고 Apollo의 환경은 실제 실행 상태와 제약을 다룹니다. 따라서 `dev`라는 이름만으로 Staging이 되거나 `main` 병합만으로 Prod 배포가 끝나지는 않습니다. 실제 통합 시험과 배포·관찰 절차가 연결돼야 합니다.

[가재코드(gajae-code)](https://github.com/Yeachan-Heo/gajae-code)는 이 관점을 구체적으로 살펴볼 수 있는 코딩 에이전트 개발 도구입니다. [기여 지침의 해당 revision](https://github.com/Yeachan-Heo/gajae-code/blob/9da99cdd708ce3b97d64111d8eefc98a7e0921ee/CONTRIBUTING.md)은 일반 PR을 `dev`로 보내고 `main`은 관리자가 지시하는 릴리스 흐름에 남기도록 안내합니다. 확인한 PR에는 `dev` 대상과 `main` 대상이 모두 있었습니다. 이 관찰은 해당 시점의 기록이며 모든 PR이 같은 경로를 거친다는 보장은 아닙니다.

`dev`에 여러 작업을 모아 CI와 리뷰를 수행하고, 충돌·회귀가 드러나면 수정과 재검증으로 돌아갑니다. 개별 작업의 완성을 통합된 결과의 검증으로 이어가는 자리입니다. 같은 검사를 형식적으로 두 번 실행하는 것보다, 다음 단계에서 무엇을 더 확인할지 정하는 일이 중요합니다.

변경 생성 속도가 검토·통합 속도를 앞서는 상황이라면 병목은 무엇을 받아들일지 판단하는 쪽으로 이동할 수 있습니다. 이는 생산성을 측정한 결론이 아니라 운영상의 가설입니다. 그렇다면 PR에는 수정 코드와 함께 어떤 근거가 있어야 할까요?
...
변경을 검증하고 운영하는 과정에는 비용이 듭니다. 현업 담당자의 리허설 시간, 승인 책임, 관측 신호, 복구 절차와 기록을 유지해야 합니다. 영향이 작고 쉽게 되돌릴 수 있는 작업까지 같은 깊이로 심사하면 변경이 쌓이고 현업이 절차를 피해 갈 수 있습니다. 반대로 잘못된 기준이 실제 출하나 외부 업무를 바꾼다면 코드 테스트만으로 충분하지 않습니다.
...
[^note13]: 『팔란티어 파운드리, 판단을 설계하라』 CHAPTER 13 「아폴로와 운영 환경」.
