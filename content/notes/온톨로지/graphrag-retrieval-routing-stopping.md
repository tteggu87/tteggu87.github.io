...
## 좋은 GraphRAG는 근거 공백을 닫고 멈출 줄 압니다

GraphRAG를 운영할 때 가장 어려운 일은 질문에 답하려면 무엇을 증명해야 하는지 끝까지 추적하는 것입니다. 그래프를 얼마나 멀리 따라갔는지는 그다음 문제입니다. 장애 원인과 영향을 받은 고객군을 함께 묻는 질문처럼, 한 문장 안에서도 배포 기록·오류 시계열·고객 로그·반례가 서로 다른 검색 경로에 흩어질 수 있습니다. 홉 수나 신뢰도 점수 하나만 보고 멈추면 관련 자료는 많아도 핵심 근거가 빈 답을 만들 수 있고, 이미 충분한데 검색을 계속하면 비용과 잡음만 늘어납니다.

실행은 **답변 의무**에서 시작합니다. 질문을 검증 가능한 의무로 나누고, 각 의무에 맞는 가장 얇은 초기 경로를 가설로 고릅니다. 이어서 그 검색기가 이해하는 경로별 질의 계약(query contract)으로 질문을 바꾸되, 원래 의미가 보존됐는지 검사합니다. 검색이 진행되는 동안에는 개체(entity)·관계(relation)·출처(source)·반례 관점에서 남은 근거 공백을 기록하고, 출처 검증 관문(source gate)을 통과한 자료로만 공백을 닫습니다.

초기에 약해 보인 후보를 바로 없애서도 안 됩니다. 보류한 후보의 상태를 추적하는 생존 원장에는 왜 후보를 보류했는지, 어떤 새 관계나 반례가 나타나면 다시 검토할지를 남깁니다. 현재 경로가 막히면 출발점을 고치는 rescue, 빠진 연결을 제한적으로 수리하는 repair, 문서나 키워드(lexical) 검색으로 옮기는 fallback, 일시적 실행 실패를 다시 시도하는 retry를 구분합니다. 가능한 경로를 확인해도 핵심 근거가 채워지지 않으면 답을 꾸며 내지 않고 답변 보류(abstain)로 끝냅니다.

이 제어 구조의 실무적 가치는 실패 원인을 되짚을 수 있다는 데 있습니다. 실행 영수증(receipt)을 보면 처음부터 경로를 잘못 골랐는지, 질의를 바꾸며 의미를 잃었는지, 필요한 후보를 너무 일찍 버렸는지, 근거가 비었는데도 답했는지를 구분할 수 있습니다. 또한 그래프 런타임을 쓰지 않는 graph-free 기준선과 같은 조건에서 검색 경로(route)·질의 변환기(query adapter)·가지치기(pruning)·중단 규칙(stopping)을 하나씩 비교해야 그래프가 실제로 답에 기여했는지도 판단할 수 있습니다. 그래프 경로를 실행했다는 사실이나 고유 후보를 찾았다는 사실만으로는 그 기여가 증명되지 않습니다.
