...
![업무의 데이터·판단·액션과 Apollo의 소프트웨어 변경을 나누고 Recall과 rollback Plan을 별도로 표시한 책임 경계](../../attachments/palantir-apollo-operating-change/palantir-apollo-operating-change-figure-06.png)

_출하를 승인하는 일과 출하 관리 서비스를 배포하는 일은 서로 다른 책임입니다. 이 그림은 역할을 나눈 설명도이며, 소프트웨어 롤백만으로 이미 수행한 업무가 취소된다는 뜻은 아닙니다._
...
![생애주기·실행 위치·연결 상태의 독립된 세 축과 운영·온프레미스·망분리의 설명용 조합 예시](../../attachments/palantir-apollo-operating-change/palantir-apollo-operating-change-figure-04.png)

_각 열은 순서대로 거치는 단계가 아니라 함께 확인할 서로 다른 조건입니다. 조합 예시는 이해를 돕기 위한 것이며, Apollo의 공식 분류나 모든 조합의 지원을 인증하는 표는 아닙니다._
...
![시험 환경·Entity의 적용 범위, 관측 신호, 확대·중단 조건, 소프트웨어와 업무 복구 책임을 함께 검토하는 네 가지 질문](../../attachments/palantir-apollo-operating-change/palantir-apollo-operating-change-figure-05.png)

_네 가지는 조직이 배포 전에 합의할 운영 설계 질문이지 Apollo의 기본 승인 정책은 아닙니다. 작은 범위로 시작하더라도 확대할 근거와 멈춘 뒤의 처리 책임은 따로 정해야 합니다._
