...
이 글에서 사용하는 **작업 권한 임대(Task Authorization Lease)**는 MCP Tasks, OpenID CAEP, OAuth Token Introspection의 공식 계약을 프로젝트의 장기 작업 구조에 연결한 설계 제안입니다. 일정 시간 동안 이전 권한 판단을 재사용하되, 시간이 끝나거나 권한 변경 신호가 오면 다시 검사한다는 뜻입니다. 확립된 표준 명칭이나 현재 DuckCrab에 구현 완료된 기능은 아닙니다. 합성 상태 전이 검사는 수행했지만 실제 MCP 서버·권한 엔진·분산 작업 대기열·완료 알림 환경의 방어 성능은 아직 측정하지 않았습니다.
...
장기 실행 작업에서는 사용자 신원, 조직 소속, 문서 권한, 도구 실행 권한이 도중에 바뀔 수 있습니다. 취소 뒤에도 실행기가 뒤늦게 쓰기를 시도할 수 있고, 완료 결과는 처음과 다른 연결에서 조회될 수 있습니다. 따라서 작업이 오래 이어지는 동안 권한도 별도의 수명주기로 추적해야 합니다.

MCP 작업의 TTL과 `working`·`completed`·`cancelled` 상태는 작업과 결과의 보존·진행 상태를 표현할 뿐, 현재 문서·그래프·도구·결과 공개 권한을 증명하지 않습니다.
