...
같은 질문, 같은 모델, 같은 Pack 버전과 같은 **입력 예산(최대 토큰 수)**을 유지하고 전달 문맥만 바꾸면 어느 단계에서 정보가 빠졌는지 더 쉽게 구분할 수 있습니다. 조건마다 실제 입력 길이는 달라질 수 있으며, 길이를 억지로 맞추려고 내용을 더 자르거나 빈 내용을 채우지 않습니다.
