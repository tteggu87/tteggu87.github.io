...
L0부터 L5까지의 사다리를 성숙도 점수로 사용하면 또 다른 문제가 생깁니다. 모든 기능이 L5를 필요로 하지는 않습니다. 문서 포맷터처럼 부작용이 작고 결과를 쉽게 검토할 수 있는 기능은 L1~L2로도 충분할 수 있습니다. 반면 권한을 바꾸거나 돈을 이동하는 에이전트는 제한 운영, 사람 책임과 복구 증거 없이는 자동화 범위를 넓히기 어렵습니다.

필요한 단계는 시스템 이름이 아니라 **주장과 실패 비용**이 정합니다.
...
이 순서를 지키면 합성 검사의 가치도 더 선명해집니다. 합성 검사는 실제 성능의 축소판이 아니라, 다음 비싼 평가로 넘어가기 전에 계약의 빈칸을 싸게 찾는 도구입니다. 오프라인 benchmark는 운영의 모형이 아니라 고정 조건의 비교 장치입니다. Shadow와 canary는 최종 인증이 아니라 실제 실패를 제한된 범위에서 발견하는 장치입니다. 배포 후 모니터링은 평가가 끝난 뒤 붙이는 대시보드가 아니라, 새로운 증거가 생겼을 때 주장을 낮추고 게이트를 다시 여는 마지막 평가 계층입니다.
