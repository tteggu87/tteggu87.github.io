...
검색 시스템은 보통 후보의 관련도를 계산합니다. 권한 시스템은 특정 주체가 특정 자원에 어떤 행동을 할 수 있는지 판단합니다. 지식 검증은 자료의 출처와 신뢰 수준을 확인합니다. 생성 검증은 모델이 허용되고 신뢰할 수 있는 근거를 실제 답에 충실하게 사용했는지 확인합니다.
...
| 게이트        | 묻는 질문                                                | 실패 예시                                       |
| ------------- | -------------------------------------------------------- | ----------------------------------------------- |
| Identity      | 요청자는 누구입니까                                      | client가 보낸 email·tenant 문자열을 그대로 신뢰 |
| Delegation    | 이 에이전트가 이 작업에서 사용자를 대신할 수 있습니까    | 다른 작업의 grant 재사용                        |
| Authorization | 이 자원·경로·행동을 지금 허용합니까                      | 허용 seed에서 금지 graph 이웃으로 확장          |
| Trust         | 자료의 출처·권위·revision을 믿을 수 있습니까             | 허용된 문서가 오염된 지시를 포함                |
| Relevance     | 허용되고 신뢰할 수 있는 후보 중 무엇이 질문에 유용합니까 | top-k를 공개 가능 목록으로 오인                 |
| Generation    | 모델이 허용된 근거를 충실하게 사용했습니까               | 금지된 이전 상태나 파라미터 기억을 섞음         |

## 권한을 계산하기 전에 자원을 먼저 식별하고 분류합니다

권한 엔진은 정책이 참조할 수 있는 자원만 판정할 수 있습니다. 원문 문서에는 분류 레이블이 있어도, 그 문서에서 만든 chunk·embedding·graph evidence·요약·citation bundle·tool schema에는 같은 경계가 전달되지 않을 수 있습니다. NIST의 2026년 데이터 분류 초안은 비정형 데이터를 발견하고 식별하며 지속 가능한 레이블을 부여하는 일을 Zero Trust 데이터 보호의 준비 단계로 둡니다.[src_029](#src-029)
...
이 기본값은 NIST가 직접 제시한 RAG 표준이 아니라 데이터 분류 원칙을 프로젝트 권한 계약에 적용한 제안입니다. 중요한 점은 ingest나 projection에서 레이블이 빠진 자원을 자동 공개로 승격하지 않는 것입니다. 레이블이 존재하더라도 원본·index·chunk·cache의 revision이 일치하는지, 실제 호출 API가 해당 레이블을 집행하는지도 따로 확인해야 합니다.[src_033](#src-033)

## 유효한 OAuth 연결 하나로는 부족합니다

MCP Authorization 명세는 HTTP MCP 서버를 OAuth 보호 자원으로 다루며, 자격증명을 특정 resource audience에 묶는 방법을 설명합니다. 또한 MCP client에서 받은 자격증명을 downstream API에 그대로 전달하는 token passthrough를 금지합니다.[src_017](#src-017) RFC 9728은 보호 자원 메타데이터와 authorization server 탐색을 표준화합니다.[src_021](#src-021)

하지만 연결이 성공했다는 사실은 문서와 도구 행동의 권한을 자동으로 증명하지 않습니다. 하나의 에이전트 요청에는 최소한 세 종류의 증명이 필요합니다.

1. **Transport proof:** 이 자격증명은 현재 MCP resource를 대상으로 발급됐습니까?
2. **Delegation proof:** 이 agent가 이 task와 session에서 이 사용자를 대신하도록 명시적으로 묶였습니까?
3. **Resource/action proof:** 이 task와 agent가 이 문서·graph hop·tool resource·action을 지금 사용할 수 있습니까?
...
OpenFGA의 task 기반 패턴은 사용자의 권한과 task의 제한된 grant를 별도로 검사합니다. task에는 필요한 tool과 특정 resource만 연결하고 만료 시간, session, 호출 범위와 agent binding을 둘 수 있습니다.[src_019](#src-019) 이 경계가 없으면 사용자가 문서를 읽을 수 있다는 이유로 agent가 그 문서를 외부에 공유하거나 삭제할 권한까지 얻을 수 있습니다.
...
MCP의 최신 client guidance는 많은 도구를 한꺼번에 모델에 넣는 대신 `catalog → inspect → execute`로 점진적으로 발견하는 패턴을 설명합니다. access-control filtering도 custom discovery가 필요한 사례로 명시합니다.[src_030](#src-030)
...
생성된 script나 workflow를 승인한 것도 내부의 모든 runtime tool call을 일괄 승인한 뜻이 아닙니다. host broker는 각 호출, 대상 resource와 cross-server data flow를 현재 grant에 대조해야 합니다. 넓은 scope를 미리 주기보다 baseline discovery·read 권한에서 시작해 write·share·delete·외부 전송이 필요한 시점에 정확한 resource·action으로 짧게 step-up하는 편이 감사와 회수에 유리합니다.[src_030](#src-030) [src_031](#src-031)
...
문서는 최신인데 ACL이 오래됐거나, ACL은 최신인데 chunk·embedding·graph projection이 이전 문서를 가리킬 수 있습니다. `content_revision`과 `permission_revision`을 분리하면 권한 회수 뒤에도 오래된 chunk가 남아 있는지 찾기 쉬워집니다.
...
OpenFGA는 허용 문서 ID를 먼저 구한 뒤 검색 필터에 넣는 pre-filter와, 더 넓은 후보를 검색한 뒤 권한 엔진으로 제거하는 post-filter를 설명합니다.[src_002](#src-002)
...
마지막 상태에서 남아 있는 후보를 반환하면 fail-open이 됩니다. Azure AI Search의 query-time ACL preview는 권한 평가 의존성이 실패할 때 부분 필터 결과 대신 오류를 반환하는 fail-closed 동작을 문서화합니다.[src_032](#src-032) 이는 모든 제품의 표준이 아니라, 권한 장애와 검색 공백을 같은 빈 결과로 숨기지 말아야 한다는 운영 사례입니다.

전체 index를 조사해야 하는 운영 경로도 평상시 자격증명을 넓히는 방식으로 만들지 않습니다. 별도 역할, 명시적 intent, 짧은 만료, 좁은 scope와 감사 기록을 가진 break-glass 경로로 분리합니다. elevated read로 얻은 자료를 답변·export·다른 tool로 전달할 때는 다시 승인해야 합니다.[src_032](#src-032) [src_033](#src-033)

### graph expansion은 hop마다 다시 검사합니다

허용된 chunk가 공유 entity와 연결됐다고 해서 그 entity의 모든 이웃을 볼 수 있는 것은 아닙니다. vector-to-graph 전환을 공격 표면으로 다룬 연구와 공개 재현 저장소는 확장 경계에 별도 권한 검사가 필요하다는 설계 근거를 제공합니다.[src_008](#src-008) [src_009](#src-009)
...
W3C PROV-O는 산출물이 어떤 entity에서 파생됐는지 표현할 수 있지만, 부모의 권한을 파생물에 어떻게 상속하고 합성할지는 정하지 않습니다.[src_022](#src-022) OpenFGA Conditions·Contextual Tuples와 SpiceDB Caveats는 시간·활성 조직·request context처럼 동적인 조건과 불완전한 판단 상태를 표현할 수 있습니다.[src_023](#src-023) [src_024](#src-024) [src_025](#src-025)

이 프로젝트에서는 이를 **Derived Artifact Authorization Closure**라는 조건부 설계 제안으로 정리했습니다.
...
권한 기능을 제품 단위의 `지원/미지원` 체크박스로 관리하면 실제 호출 경로의 차이를 놓칩니다. 공식 문서를 비교하면 같은 Azure AI Search 안에서도 일반 Search POST와 knowledge-base retrieve의 end-user identity 누락 기본값이 다를 수 있습니다. 최근 Search POST preview는 보호 문서를 제외하는 경로를 설명하지만, knowledge-base retrieve는 identity token이나 permission metadata가 빠지면 결과가 unfiltered일 수 있다고 명시합니다.[src_032](#src-032) [src_037](#src-037)
...
최종 답변만 저장하면 어떤 principal, policy revision, graph path와 tool action이 허용됐는지 재현하기 어렵습니다. `AuthorizationReceipt`는 다음과 같은 최소 정보를 남기는 프로젝트 제안입니다.
...
실제 tenant 데이터와 credential을 사용하지 않고, 열 가지 실패 클래스를 점진적인 방어 조건에 배치한 결정론적 스크립트를 실행했습니다.
