...
LLM Wiki를 만들겠다고 빈 폴더를 열면 대개 검색 엔진부터 고민합니다. 임베딩 모델, vector DB, graph DB, reranker를 고르다 보면 정작 더 위험한 질문이 남습니다. **모델이 방금 만든 문장을 어느 파일에 써도 되는가?**
...
`llm-wiki-from-scratch`의 네 객체 권위 모델과 `persistent-knowledge-compiler`의 provenance·승격·rollback 요구를 연결해, 파일 기반 fixture로 수명주기 8개 검사를 실행했습니다. 검사는 8/8 통과했지만 이는 작은 결정론적 파일 흐름의 성공입니다. LLM 추출 품질, 검색 정확도, 비용, 지연, 보안 격리와 대규모 운영은 측정하지 않았습니다.

## 1. 권위를 네 종류로 나누는 이유

[1번 글](../../notes/llm-wiki/llm-wiki-origin-and-implementations)은 Andrej Karpathy의 원형을 `Raw Sources / Markdown Wiki / Schema`로 정리했습니다. 이 구조는 무엇을 보존하고 무엇을 종합할지 설명하지만, 자동화가 파일을 쓰기 시작하면 상태 전이가 더 필요합니다.
...
가운데 단계를 생략하면 모델 출력이 곧 Wiki가 됩니다. 반대로 모든 결과를 임시 메모로만 남기면 장기 지식이 자라지 않습니다. 그래서 첫 구현에서는 저장 형식보다 권위를 네 종류로 나눕니다.

- `source` — 원문·직접 관찰과 revision. 수집기·사람이 작성하며, 수집된 원문 revision의 기준 기록입니다. 진실성을 보증하지 않으며 candidate의 근거가 됩니다.
- `candidate` — 아직 승인하지 않은 주장·요약·관계. Agent·사람이 작성하며, reviewed·rejected·superseded 상태로 이동합니다.
- `canonical` — 검토된 현재 지식 revision. reviewer·promotion 명령이 작성하며, 사람이 읽는 현재 정본입니다. 새 revision이 나오거나 deprecated 될 수 있습니다.
- `receipt` — 한 질문·실행의 선택·제외·missing 기록. query runtime이 작성하며, 필요하면 새 candidate의 출발점이 됩니다.
...
Source 파일은 같은 원문 revision의 같은 구간으로 돌아갈 주소를 남깁니다. 요약이 아니라, 나중에 주장을 다시 검사할 수 있는 출발점입니다.
...
`path`만 저장하면 파일이 바뀐 뒤에도 같은 근거라고 착각할 수 있습니다. hash만 저장하면 사람이 어느 문단을 읽어야 하는지 알기 어렵습니다. 첫 MVP에서도 `revision + hash + locator`를 함께 둡니다.

Locator의 최적 형식은 문서 종류에 따라 다릅니다. Markdown heading, paragraph ID, PDF page·bbox, 표의 row key, 영상 timestamp 중 무엇이 장기적으로 가장 안정적인지는 이 글에서 비교하지 않았습니다. 중요한 것은 locator가 실제로 해석되는지 검사하는 것입니다.

## 3. Candidate는 Agent의 쓰기 자유가 머무는 곳

Agent가 canonical을 직접 수정하지 못하게 하면 자동화가 쓸모없어 보일 수 있습니다. 실제로 필요한 것은 쓰기 금지가 아니라 쓰기 경로의 분리입니다.

Agent가 할 수 있는 일은 source 등록 제안, claim candidate 생성, 반례·unknown candidate 생성, receipt 작성입니다. 바로 할 수 없는 일은 canonical claim 교체, policy·permission 확대, 기존 반례 삭제, 승인 없이 source scope보다 넓게 공개하는 것입니다.
...
## 5. Receipt는 답변을 되짚는 기록

Receipt는 답변 전문을 저장하는 로그보다 좁고, 근거를 재구성하는 기록보다 넓습니다. 최소 필드는 질문, 선택한 근거, 제외한 후보, missing, 답변 hash입니다.
...
Receipt가 필요한 이유는 성공한 답변을 자동으로 학습시키기 위해서가 아닙니다. 잘못된 답이 발견됐을 때 다음을 구분하기 위해서입니다.
...
Agent 역할로 candidate 두 개를 제안했습니다. 하나는 "승격 전에 reviewer가 기존 locator를 확인해야 한다"로 Rule A가 직접 지지합니다. 다른 하나는 "반복된 receipt는 review 없이 자동 승격할 수 있다"로 Rule B와 맞지 않는 범위 과장입니다.

Review 결과 첫 번째만 `canonical-rev1`에 들어갔고 두 번째는 rejected로 남았습니다. 이어서 질문을 실행해 승인된 revision과 locator를 selected에, 기각된 후보를 excluded에 기록한 receipt를 만들었습니다.

실행 결과는 8개 검사 전체 통과입니다.

- source hash·revision 등록
- Agent의 canonical 직접 쓰기 차단
- locator 근거가 있는 candidate 승격
- 근거를 넘는 candidate 기각
- query가 reviewed revision만 선택
- receipt 재구성 필드 완전성
- source 변경 시 stale candidate 생성
- 이전 canonical revision rollback 주소 보존

생성된 객체 수는 source 1개, 초기 candidate 2개, reviewed promotion 1개, rejection 1개, receipt 1개, stale proposal 1개입니다. 이 숫자는 제품 성능 지표가 아닙니다. 고정된 작은 fixture가 설계한 상태 전이를 그대로 수행했는지 확인한 카운트입니다. LLM을 호출하지 않았고 검색 품질도 평가하지 않았습니다.

## 7. 원문이 바뀌면 조용히 고치지 않는다
...
초기 query 계약도 간단합니다. 답변 후보는 reviewed canonical과 현재 source revision, 허용 visibility의 교집합입니다. candidate·rejected는 기본 제외하고, claim-heavy 답변은 locator를 재확인하며, 근거 공백은 missing으로 기록한 뒤 보류할 수 있습니다.
...
이 구조는 작고 이해하기 쉽지만 운영 비용이 따릅니다.

**Review queue가 병목이 됩니다.** Agent가 canonical을 직접 쓰지 않으면 candidate가 쌓입니다. reviewer accept·edit·reject 비율, 대기 시간, stale age를 측정하고 저위험 source note와 고위험 policy claim의 gate를 다르게 설계해야 합니다.

**Locator가 깨질 수 있습니다.** Heading 이름 변경, PDF 재편집, table row 이동은 locator를 무효화합니다. hash는 변경을 알려 주지만 새 위치를 자동으로 찾아 주지 않습니다. locator resolution 실패를 조용히 무시하지 않고 stale·missing으로 표면화해야 합니다.

**파일 권한만으로 보안을 증명할 수 없습니다.** 이 fixture의 canonical 쓰기 차단은 코드의 actor 비교로 재현한 계약 검사입니다. 실제 multi-user·multi-agent 환경의 ACL, tenant 격리, symlink·path traversal, remote write와 비밀 관리는 검증하지 않았습니다.

**Markdown만으로 모든 규모를 감당하지 못합니다.** 동시 편집, 수백만 segment, 대규모 ACL filter, 낮은 지연의 hybrid retrieval이 필요하면 JSONL registry, SQLite·vector index, graph projection과 별도 review service가 필요할 수 있습니다. 다만 이 파생 계층도 source와 canonical revision에서 재생성 가능해야 합니다.

## 10. 확장의 승격 기준

기능 목록으로 확장하지 않고 관찰된 실패 증거로 확장합니다.

- exact·BM25가 표현 차이로 관련 page를 반복 누락 → local embedding·hybrid RAG (비교 기준선: lexical only)
- 관계·영향 질문이 wikilink 1~2 hop에서 반복 실패 → typed derived graph (기준선: Wiki navigation + source verification)
- entity alias·relation domain/range 충돌이 반복 → 경량 ontology·schema guard (기준선: registry 없는 구조)
- candidate가 너무 많아 review가 정체 → risk tier·batch review·dedupe (기준선: 단일 queue)
- source 갱신 뒤 stale page를 놓침 → dependency registry·incremental invalidation (기준선: 수동 audit)
- 여러 agent가 보호 파일을 건드림 → write allowlist·proposal service·approval role (기준선: 파일 권한만 사용)
...
이 여덟 단계가 재현되기 전에는 검색 기능이 부족한지 권위 경계가 부족한지 구분하기 어렵습니다.
...
이 흐름이 안정되면 vector, graph, ontology는 "있으면 멋진 기술"이 아니라 관찰된 실패를 해결하는 파생 계층이 됩니다. 첫 MVP에서 다음 계층을 고민하기 전에, 지금 실패가 어느 경계에서 나오는지 먼저 확인해 보는 것을 권합니다.
...
- [2. DocTology LLM Wiki 해부학: 폴더 구조와 실행 경계를 함께 읽는 법](../../notes/llm-wiki/doctology-llm-wiki-anatomy)
