...
RAG가 익숙해진 시대에 Andrej Karpathy가 건넨 `LLM Wiki`는 바로 이 빈틈을 건드립니다. 답변을 한 번 생성하고 끝내는 대신, 다음 질문에서 다시 쓸 판단과 연결을 사람이 읽고 고칠 수 있는 Wiki에 남기자는 제안입니다. 그렇다면 LLM Wiki는 새로운 검색기가 아니라, 검색 이후의 지식을 어떻게 오래 살릴 것인지에 대한 아이디어가 됩니다.
