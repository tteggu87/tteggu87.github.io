...
먼저 이 글에서 말하는 **LLM Wiki**부터 분명히 하겠습니다. 일반 위키가 사람이 지식을 문서와 링크로 정리해 함께 쓰는 공간이라면, LLM Wiki는 LLM이나 에이전트가 보고서·로그·논문 같은 원문을 읽고 다음 작업에서 재사용할 내용을 페이지로 정리하는 구조입니다. 여기에는 개념의 뜻과 함께 어떤 출처에서 나온 판단인지, 어느 버전과 조건에서 유효한지, 어떤 반례와 기각한 대안이 있었는지, 무엇이 아직 미해결인지가 담깁니다. **원문을 보존하고 되돌아갈 경로를 유지한 채, 검토된 판단을 다음 조사에 넘기는 작업 공간**이라고 이해하면 됩니다. 아직 하나의 업계 표준 제품명이나 규격은 아니며, 여러 연구와 공개 구현이 공유하는 느슨한 설계 패턴에 가깝습니다.

한 팀이 지난 반년 동안 조사 보고서 200개를 만들었다고 가정해 보겠습니다. 새 질문이 들어올 때마다 200개를 처음부터 다시 읽으면, 같은 개념을 다시 정리하고 이미 기각한 가설을 또 검토하게 됩니다. LLM Wiki는 이 반복을 줄이기 위해 출처 메모, 개념 설명, 비교 분석, 결정 이유와 열린 질문을 서로 연결해 둡니다. 다음 에이전트는 관련 페이지에서 이전 판단의 맥락을 파악하고, 최신성이 필요한 부분만 원문이나 애플리케이션 프로그래밍 인터페이스(Application Programming Interface, API)로 다시 확인할 수 있습니다.

이제 장점과 위험이 함께 보입니다. 이전 조사를 재사용할 수 있지만, 원문을 페이지로 정리하는 순간 중요한 조건이 빠질 수 있습니다. 질문에 필요한 페이지 몇 개를 다시 고르는 과정에서도 또 한 번 정보가 줄어듭니다.

예를 들어 보고서를 Wiki로 정리하면서 “이 결론은 특정 버전에서만 유효하다”는 조건이 빠질 수 있습니다. 그 뒤 Wiki에서 질문에 필요한 몇 쪽을 고를 때는 남아 있던 반례마저 제외될 수 있습니다. 마지막 답이 그럴듯하면 어느 단계에서 맥락을 잃었는지 알아보기 어렵습니다.

이 글에서는 이 두 변환을 **이중 컴파일**이라고 부르겠습니다.

여기서 질문별 문맥 묶음(Context Bundle)은 현재 질문에 필요한 주장·근거·반례·보류 조건만 골라 모델에 전달하는 입력 묶음입니다.
...
원문·로그·보고서
  └─ 1차 컴파일: 유지보수 시점
       └─ 검토된 지속 Wiki
            └─ 2차 컴파일: 질문 시점
                 └─ 질문별 문맥 묶음(Context Bundle)
                      └─ 답변·행동
```

이 명칭은 확립된 업계 표준이 아니라 문제를 나누기 위한 프로젝트 용어입니다. 서로 목적이 다른 두 축약 단계를 구분하고, 답이 잘못됐을 때 실패 위치가 섞이지 않게 하려는 진단 틀입니다.
...
## 1. LLM Wiki의 역할을 알아야 RAG와 비교할 수 있습니다

LLM Wiki가 주로 하는 일은 조사 과정에서 얻은 지식을 **다음 조사에서도 이어서 쓸 수 있는 형태로 남기는 것**입니다. 출처 메모에는 원문의 위치와 핵심 사실을, 개념 페이지에는 여러 자료에서 반복되는 용어와 경계를, 분석 페이지에는 비교 결과·결정 이유·기각한 대안·남은 질문을 기록합니다. 페이지끼리 링크를 연결하면 다음 작업자는 결론만 보는 데서 멈추지 않고, 그 결론을 만든 근거와 반례까지 따라갈 수 있습니다.

Andrej Karpathy가 제안한 LLM Wiki도 에이전트가 큰 자료 묶음을 탐색하면서 읽은 내용을 연결된 문서로 축적하는 패턴을 보여 줍니다.[src_001](#src-001) 다만 Wiki 페이지는 원문 자체나 자동으로 확정된 정답이 아닙니다. 누가 어떤 자료를 바탕으로 작성했는지, 언제 다시 검토해야 하는지와 어느 범위까지 믿을 수 있는지를 함께 관리해야 합니다.

RAG는 작동 시점이 다릅니다. 질문이 들어오면 검색어와 가까운 원문 조각을 찾아 현재 답변의 재료로 제공합니다. 가격, 현재 정책, 방금 바뀐 장애 상태처럼 최신성이 중요한 사실이나 한 문서의 정확한 문장을 확인할 때 자연스럽습니다.

이제야 둘을 비교할 기준이 생깁니다. LLM Wiki는 조사하면서 얻은 판단과 맥락을 오래 관리하고, RAG는 지금 질문에 필요한 원문을 그때그때 다시 찾습니다. 둘을 단순히 “검색 기술 A와 검색 기술 B”로 놓으면 이 역할 차이가 흐려집니다.
...
24개 논문과 13개 질문을 사용한 한 사전등록 비교에서는 문장을 숫자 벡터로 바꿔 가까운 원문을 찾는 vector RAG가 단일 사실 조회 기준을 충족했고, Wiki는 여러 논문을 잇는 종합과 주장 단위 인용 지원에서 강점을 보였습니다. 그러나 Wiki 질의의 토큰 사용량은 더 많았고, 질문을 여러 하위 질문으로 나눠 각각 검색하는 decomposition RAG가 더 낮은 비용으로 종합 격차의 상당 부분을 줄였습니다. 어느 한 방식도 모든 평가 축에서 이기지 못했습니다.[src_004](#src-004)

여기서 토큰은 모델이 읽거나 생성하는 텍스트를 잘게 나눠 세는 단위입니다.
...
LLM Wiki는 같은 조사와 판단을 반복하는 팀이 **검토를 마친 종합을 다음 작업의 출발점으로 남길 때** 도움이 됩니다. 검색기는 계속 필요합니다. Wiki에 적힌 내용도 현재 사실이나 정확한 문장이 중요하면 원문과 API를 통해 다시 확인해야 합니다.
...
Wiki-memory Compile, Evaluate, Refine(WiCER)는 Wiki 메모리를 만들고 평가하고 수정하는 반복 과정을 제안한 2026년 사전 인쇄 연구입니다. 이 연구는 실험한 조건에서 blind compilation, 즉 별도 진단 없이 Wiki를 만든 경우 53~60%의 치명적 실패를 보고했고, 진단 검사와 반복 수정을 통해 손실을 상당 부분 회복했다고 주장했습니다.[src_003](#src-003) 이 수치를 모든 Wiki의 실패율로 읽어서는 안 됩니다. 저자 설정에 한정된 결과입니다.
...
내부 DuckCrab에서 실행한 제한적 결정론 스모크는 이 차이를 작은 예제로 확인했습니다. 직접 원문 경로, 그래프 후보를 먼저 고르는 graph-first top-k, 역할별로 근거를 묶은 typed bundle은 미리 정한 네 가지 의무를 모두 보존했습니다. 원문 일부만 뽑는 제한된 extractive 경로와 일부러 정보를 빠뜨린 projection(투영본)은 네 의무 중 위협 모델 제한을 빠뜨렸습니다.[src_008](#src-008)

이 결과는 LLM 답변의 정확도나 특정 압축기의 우월성을 입증하지 않습니다. 모델 생성도, 학습된 압축기도, 활성 vector index도 사용하지 않았습니다. 정해 둔 합성 사례에서 계약 분기가 작동하는지만 확인한 **L1 수준의 계약 스모크**입니다. [[notes/agent-evaluation-evidence-ladder|24번 글]]의 표현을 빌리면 “작성한 사례에서 누락 분기를 잡았다”까지가 주장 상한입니다.[src_009](#src-009)

그럼에도 실무적 의미는 있습니다. 2차 컴파일 검사는 최종 답이 나오기 전에 다음 질문에 답하게 합니다.

- 질문이 요구한 Claim(주장)·Evidence(근거)·Counterevidence(반대 근거)·Policy(정책)·Unknown(미확인) 역할을 먼저 정의했는가?
...
Wiki의 링크는 **다시 확인할 경로**입니다. 관계와 경로를 그래프로 탐색하는 GraphRAG의 검증된 edge(관계선)나 웹 온톨로지 언어(Web Ontology Language, OWL)의 형식 공리와 같은 것으로 간주하면 안 됩니다. 관계 기반 질문이라면 graph traversal이 필요할 수 있고, 형식 판정에는 그래프 제약을 검사하는 Shapes Constraint Language(SHACL) 같은 결정론적 계층이 더 잘 맞을 수 있습니다.
...
## 6. 필요한 페이지만 단계적으로 여는 Progressive disclosure
...
Open Worldwide Application Security Project(OWASP)의 GenAI Security Project는 메모리, 자동 실행 연결점(hook)과 로컬 설정을 미래 세션의 추론과 행동에 영향을 주는 보안 상태로 다뤄야 한다고 설명합니다.[src_006](#src-006) 별도의 `Bad Memory` 사전 인쇄 연구는 격리된 Claude Code·Codex 실험 환경에서 세션이 끝난 뒤에도 남는 파일(persistent file)에 미리 심은 공격 문구(payload)가 현재와 미래 세션에 영향을 줄 수 있음을 보였습니다. 성공과 지속성은 시스템, 모델과 목표에 따라 달랐습니다.[src_007](#src-007)
...
## 8. 두 컴파일 단계에는 서로 다른 실행 영수증(Receipt)이 필요합니다
...
실험 등급도 분명히 합니다. 이 합성 사례가 통과했다면 계약 분기가 작동했다는 뜻입니다. 실제 corpus와 독립 gold label, 반복 실행, 동일 예산 비교와 사람 표본이 있어야 성능 주장으로 올라갈 수 있습니다. 제한 운영과 배포 후 관찰이 없으면 운영 안전성까지 말할 수 없습니다.
...
| 한 문서의 사실 확인            | 키워드 검색(BM25)·vector RAG와 원문 span |
...
LLM Wiki는 여러 원문에서 얻은 개념·결정·반례·열린 질문을 출처와 함께 연결된 페이지로 정리해, 다음 조사에서 다시 쓰도록 만드는 지속 지식 공간입니다. RAG는 지금 질문에 필요한 원문 조각을 그때그때 찾아오는 검색 방식입니다. 최신 사실이나 정확한 문장을 확인할 때는 원문 검색이 먼저이고, 여러 번 되풀이되는 비교와 판단에는 검토된 Wiki가 도움이 됩니다. 질문의 성격과 자료의 변화 속도에 따라 둘을 함께 쓰거나 더 단순한 한쪽만 선택해야 합니다.

둘을 함께 쓸 때는 편리함만 이어 붙여서는 안 됩니다. 원문을 지속 Wiki로 정리하는 1차 컴파일에서는 주장과 근거뿐 아니라 반례, 적용 범위, 유효 시점과 미지까지 보존해야 합니다. Wiki에서 현재 질문에 필요한 근거만 고른 질문별 문맥 묶음(Context Bundle)을 만드는 2차 컴파일에서는 근거 역할과 보류 조건이 남아 있는지 확인해야 합니다. 두 단계는 입력과 권한, 잃어버릴 수 있는 정보가 다르므로 각 단계의 입력 버전(revision), 선택 규칙(selector), 보존 결과를 실행 영수증(Receipt)으로 남겨야 합니다.
...
다만 이중 컴파일과 두 Receipt는 확립된 업계 표준이 아니라 이 글이 제안한 진단 틀입니다. 공개 LLM Wiki 구현들을 같은 환경에서 설치해 성능·비용·보안을 비교하지 않았고, 내부 DuckCrab 스모크도 정해 둔 합성 사례에서 누락 분기를 확인한 L1 계약 검사에 머뭅니다. 따라서 현재 근거로 말할 수 있는 것은 실패 위치를 분리해 검사할 수 있다는 설계상의 이점까지이며, 특정 구현이 RAG보다 우수하다거나 실제 조직의 시간과 비용을 줄인다는 결론은 동일 조건 평가와 제한 운영을 거쳐야 합니다.
