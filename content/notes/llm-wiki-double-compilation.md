...
> [!summary] 핵심 결론
> LLM Wiki는 에이전트가 여러 원문을 읽은 뒤, 다음 조사에서 다시 쓸 개념·결정·반례·열린 질문을 출처 링크와 함께 연결된 페이지로 정리해 두는 **지속 지식 공간**입니다. RAG는 질문이 들어온 순간 관련 원문 조각을 찾아 답변에 넣는 검색 방식입니다. 두 구조는 서로 다른 시간에 작동합니다. 함께 사용할 때는 **원문을 Wiki로 옮기는 과정과 Wiki를 질문별 문맥으로 줄이는 과정에서 무엇이 사라졌는지 각각 검사해야 합니다.**

먼저 이 글에서 말하는 **LLM Wiki**부터 분명히 하겠습니다. 일반 위키가 사람이 지식을 문서와 링크로 정리해 함께 쓰는 공간이라면, LLM Wiki는 LLM이나 에이전트가 보고서·로그·논문 같은 원문을 읽고 다음 작업에서 재사용할 내용을 페이지로 정리하는 구조입니다. 여기에는 개념의 뜻과 함께 어떤 출처에서 나온 판단인지, 어느 버전과 조건에서 유효한지, 어떤 반례와 기각한 대안이 있었는지, 무엇이 아직 미해결인지가 담깁니다. **원문을 보존하고 되돌아갈 경로를 유지한 채, 검토된 판단을 다음 조사에 넘기는 작업 공간**이라고 이해하면 됩니다.

한 팀이 지난 반년 동안 조사 보고서 200개를 만들었다고 가정해 보겠습니다. 새 질문이 들어올 때마다 200개를 처음부터 다시 읽으면, 같은 개념을 다시 정리하고 이미 기각한 가설을 또 검토하게 됩니다. LLM Wiki는 이 반복을 줄이기 위해 출처 메모, 개념 설명, 비교 분석, 결정 이유와 열린 질문을 서로 연결해 둡니다. 다음 에이전트는 관련 페이지에서 이전 판단의 맥락을 파악하고, 최신성이 필요한 부분만 원문이나 API로 다시 확인할 수 있습니다.
