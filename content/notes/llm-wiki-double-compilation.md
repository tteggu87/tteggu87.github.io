...
LLM Wiki는 RAG를 대체하는 만능 검색기가 아닙니다. RAG는 지금 필요한 원문을 찾는 데 강하고, Wiki는 여러 번 읽은 자료에서 다시 쓸 종합을 남기는 데 가치가 있습니다. 질문의 종류와 자료의 변화 속도에 따라 둘을 함께 쓰거나, 더 단순한 한쪽만 쓸 수 있습니다.

둘을 함께 쓸 때는 편리함만 이어 붙이지 말아야 합니다. 원문을 Wiki로 정리하는 1차 컴파일과, Wiki를 질문별 Context Bundle로 줄이는 2차 컴파일은 다른 입력, 다른 손실과 다른 권한을 가집니다. 하나의 최종 정확도로 덮지 말고 단계별 보존 의무와 Receipt를 남겨야 합니다.

이 글의 제안은 네 문장으로 줄일 수 있습니다.

```text
원문이 참인지 검사합니다.
Wiki가 원문의 범위와 반례를 보존했는지 검사합니다.
질문별 문맥이 필요한 의무를 유지했는지 검사합니다.
모델이 그 문맥을 실제로 사용했는지 검사합니다.
```

그래야 답이 흔들렸을 때 “Wiki가 나빴다”거나 “RAG가 약했다”고 한꺼번에 결론 내리지 않습니다. 어느 컴파일에서 무엇이 사라졌는지 찾고, 입력이 바뀌지 않은 앞 단계는 건드리지 않은 채 실패한 단계부터 다시 실행할 수 있습니다.

지속 지식의 목적은 원문을 잊는 것이 아닙니다. **다시 읽을 이유와 경로를 더 잘 남기는 것**입니다.
